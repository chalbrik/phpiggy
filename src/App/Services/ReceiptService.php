...
class ReceiptService
{
    ...
    public function validateFile(?array $file)
    {
        ...
        $maxFileSizeMB = 3 * 1024 * 2024;

        if ($file['size'] > $maxFileSizeMB) {
            throw new ValidationException([
                'receipt' => ['File uplpoad is to large']
            ]);
        }
    ...
    public function upload(array $file, int $transaction)
    {
        ...
        $fileExtesion = pathinfo($file['name'], PATHINFO_EXTENSION);

        $newFilename = bin2hex(random_bytes(16)) . "." . $fileExtesion;
